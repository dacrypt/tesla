"""Telemetry API routes: /api/teslaMate/* (backwards-compatible URL)"""
...
def _backend():
    """Return a TelemetryBackend reading from local SQLite."""
    cfg = load_config()
    db_path = cfg.teslaMate.db_path or str(Path.home() / ".tesla-cli" / "telemetry.db")
    if not Path(db_path).exists():
        raise HTTPException(
            status_code=503,
            detail="Telemetry database not found. The data logger will create it automatically.",
        )
    from tesla_cli.core.backends.telemetry import TelemetryBackend
    return TelemetryBackend(db_path=db_path, car_id=cfg.teslaMate.car_id)
...
def telemetry_status() -> dict:
    """Telemetry data logger status.

    Returns poller state, DB stats, and configuration.
    """
    from tesla_cli.api.app import _datalogger_instance
    cfg = load_config()
    db_path = cfg.teslaMate.db_path or str(Path.home() / ".tesla-cli" / "telemetry.db")

    result: dict = {
        "enabled": cfg.teslaMate.enabled,
        "running": _datalogger_instance is not None and _datalogger_instance.is_running,
        "poll_interval": cfg.teslaMate.poll_interval,
        "db_path": db_path,
        "db_exists": Path(db_path).exists(),
    }

    if Path(db_path).exists() and _datalogger_instance:
        try:
            result["db_stats"] = _datalogger_instance.get_db_stats()
        except Exception:
            pass
