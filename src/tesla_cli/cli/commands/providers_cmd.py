...
def providers_status() -> None:
    ...
    console.print()
    console.print("  [bold]Capability routing[/bold] [dim](→ highest-priority available provider)[/dim]\n")
    ...
    for cap in sorted(all_caps):
        try:
            ...
            if providers_for_cap:
                console.print(f"  [dim]{cap:<26}[/dim]  [yellow]→ configured ({', '.join(providers_for_cap)}) but unavailable[/yellow]")
            else:
