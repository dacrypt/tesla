...
OPTION_CODE_MAP: dict[str, tuple[str, str, str]] = {
    ...
    "IPB12": (
        "interior",
        "Premium Black Gen 12 (Juniper)",
        "Interior Premium Negro Gen 12 (Juniper)",
    ),
    "IPW0": ("interior", "Premium White", "Interior Premium Blanco"),
    "IPW1": ("interior", "Premium White", "Interior Premium Blanco"),
    "IPW12": (
        "interior",
        "Premium White Gen 12 (Juniper)",
        "Interior Premium Blanco Gen 12 (Juniper)",
    ),
    ...
    "SC05": (
        "charging",
        "Free Supercharging (transferable)",
        "Supercharging Gratis (transferible)",
    ),
    ...
    "AU3P": (
        ...
        "Sistema de Sonido Ultra Alta Fidelidad",
    ),
    ...
    "FR01": (
        ...
        "Asientos Deportivos Delanteros (calefacción)",
    ),
    "FR02": (
        "feature",
        "Premium Front Seats (heated + ventilated)",
        "Asientos Premium Delanteros (calef. + ventilación)",
    ),
    ...
    "MR32": (
        ...
        "Espejos Atenuación Automática + Calefacción",
    ),
...
class DossierBackend:
    ...
    def build_dossier(self) -> VehicleDossier:
        ...
        if nhtsa:
            dossier.raw_snapshots.append(
                {"source": "nhtsa", "timestamp": str(datetime.now()), "data": nhtsa}
            )
    ...
    def _save_dossier(self, dossier: VehicleDossier) -> None:
        ...
        DOSSIER_FILE.write_text(dossier.model_dump_json(indent=2, exclude_none=True))
    ...
    def _save_snapshot(self, dossier: VehicleDossier) -> None:
        ...
        path.write_text(dossier.model_dump_json(indent=2, exclude_none=True))
